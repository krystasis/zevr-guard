# 再レビュー指摘の修正(2026-09-14)— **対応済み**

> **結果**: A1〜A7 / B1〜B5 / C をすべて実装し、コミット `61ea84e`(A・B)と `242f7b6`(C)に入っている。単体 153 件、実機 84/84(3 回連続)、Chrome / Firefox ビルド通過。以下は当時の指示内容で、記録として残す。
>
> 実装中に判明した追加の不具合が 1 件: A4 の修正で main_frame の完了時に訪問を記録するようにしたところ、**拡張自身のページ(警告ページ・ポップアップ)まで記録**され、しかも訪問マップは丸ごと書き戻すため実データを上書きしていた。http(s) に限定して解決済み。実機ハーネスが検出した。
>
> C のうち「`AllowAndOpen` と `ContinueOnce` を 1 コンポーネントに」は `scope` プロパティで統合したが、他の C 項目と違い見た目の判断が入るので、レビュー時はそこを重点的に見ること。
