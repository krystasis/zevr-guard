...
> **対応済み(2026-09-15)**: 必須 2 件・任意 A/B/C/D をすべて実装。単体 170 件、実機 122/122(3 回連続)。残るは E(サイトのビルド確認)と、push・Worker デプロイ・フィード再配信・版上げ。以下は当時の指示で、記録として残す。
>
> 必須 2 を直したところ **その場で新たな実害を検出した**: 公開フィードは 2,488 件通るのにセッションルールは 2,400 件までで、差分 88 件が「危険と判定されるがブロックされない」状態だった。サーバーの件数に依存しないよう、クライアント側でも `FEED_MAX_DOMAINS` で切るようにした(`sanitizeMalwareFeed`)。空振りしていた検査を本物にした直後に見つかったもので、必須 2 の価値がそのまま出た形。
