...
- **用途**: `scripts/safelist.ts` が、脅威フィード(URLhaus / ThreatFox)から人気サイトとその配下を除外するために使う。上位 10,000 件を `src/data/tranco.snapshot.json`(登録ドメインの配列、順位順)として保持。
- **規約**: 研究・商用とも利用可、出典表記が条件(Le Pochat et al., NDSS 2019)。取得は zip を 1 回落とすだけで、スクレイピングはしない。
- **更新**: 数か月に 1 度で足りる(人気サイトの顔ぶれは安定)。更新時は `python3 -c` で CSV 2 列目の上位 10,000 を JSON 配列に落とし、`npm test` の `scripts/safelist.test.ts` が通ることを確認する。
